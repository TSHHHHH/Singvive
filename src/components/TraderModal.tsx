      ...
      >
        <div className="shrink-0 border-b border-white/10 p-4">
          <div className="flex items-center justify-between">
            <h3 className="text-read font-bold" style={{ color: cfg.color }}>
              <Icon name={cfg.icon} /> {cfg.shortName} {cfg.outpostName}
            </h3>
            <button onClick={closeTrader} className="text-body text-white/40 hover:text-white/70">
              {t('ui.common.close')}
            </button>
          </div>
          <p className="mt-2 text-body italic text-white/50">{trader.greeting}</p>
          <div className="mt-2 flex items-center gap-2 text-label uppercase">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-3">
          {trader.offers.length === 0 ? (
            <p className="p-4 text-center text-read text-white/30">{t('ui.trader.bare')}</p>
          ) : (
        ...
        <p className="shrink-0 border-t border-white/10 px-4 py-2 text-micro text-white/30">
          {t('ui.trader.footer')}
        </p>
      </div>
    </div>
...
function OfferRow({
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 text-read">
      ...
      <div className="mt-2 flex items-center justify-between gap-2">
        ...
        <span className={`text-micro ${afford ? 'text-white/40' : 'text-hiss'}`}>
          {t('ui.trader.carrying', { n: held })}
        </span>
        <button
          disabled={taken || !afford}
          onClick={onAccept}
          className="rounded px-3 py-1 text-body font-semibold text-black disabled:cursor-not-allowed disabled:bg-white/10 disabled:text-white/30"
