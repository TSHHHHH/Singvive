 ...
 *   2. `item.<id>`, if that key exists in emoji map or has a discovered asset —
 *      so declaring the key and/or dropping `src/assets/icons/item-<id>.png`
 *      is enough to give an item its own art,
 *   3. the effect-kind fallback, so a brand-new item is never blank.
 */
export function itemIcon(def: ItemDef): IconName {
  if (def.icon) return def.icon;
  const own = `item.${def.id}` as IconName;
  if (ITEM_KEYS.has(own) || ICON_ASSETS[own]) return own;
