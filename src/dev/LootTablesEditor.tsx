...
export function LootTablesEditor({
  ...
  if (!tables) {
    return <p className="p-6 text-read text-white/40">{busy ? 'Loading…' : 'No tables loaded.'}</p>;
  }

  return (
    <div className="relative flex min-h-0 flex-1">
      <aside className="flex w-64 shrink-0 flex-col border-r border-white/10 bg-black/20">
        <div className="border-b border-white/10 p-3 text-label uppercase text-white/30">
          POI categories
        </div>
            ...
            return (
              <li key={cat}>
                <button
                  ...
                  className={`flex w-full flex-col items-start gap-1 px-3 py-2 text-left text-read transition ${
                    active ? 'bg-signal/15 text-signal' : 'text-white/75 hover:bg-white/5'
                  }`}
                >
                  <span className="font-semibold">{POI_CONFIG[cat].label}</span>
      ...
      <div className="flex min-h-0 min-w-0 flex-1 flex-col">
        <div className="flex flex-wrap items-center gap-2 border-b border-white/10 px-4 py-3">
          <div className="mr-auto min-w-0">
            <h4 className="text-title text-signal">{POI_CONFIG[category].label}</h4>
            <p className="text-body text-white/40">{POI_CONFIG[category].blurb}</p>
          </div>
          {dirty && (
        ...
        <div className="flex flex-wrap items-end gap-2 border-b border-white/10 px-4 py-3">
          <label className="flex min-w-[10rem] flex-1 flex-col gap-0.5 text-body">
          ...
          <label className="flex min-w-[12rem] flex-[2] flex-col gap-0.5 text-body">
          ...
          <button
            type="button"
            onClick={addRow}
            className="rounded border border-signal/40 px-2.5 py-1.5 text-body text-signal"
          >
            Add row
          </button>
          <label className="flex flex-col gap-0.5 text-body">
          ...
          <div className="pb-1 text-micro text-white/35">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto">
              ...
              {displayRows.map(({ row: [id, weight], index }) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-2 py-2">
                      <div className="flex items-center gap-2">
                        ...
                        <div className="min-w-0">
                          <div className="flex flex-wrap items-center gap-1.5 truncate font-semibold text-white/85">
                            <span>{def?.name ?? id}</span>
                            {!def && (
                              <span className="rounded bg-red-500/20 px-1 text-micro text-red-300">
                                unknown
                              </span>
                            )}
                            {def?.exotic && (
                              <span className="rounded bg-violet-500/20 px-1 text-micro text-violet-200">
                                exotic
                              </span>
                            )}
                            {def?.startingItem && (
                              <span className="rounded bg-amber-500/20 px-1 text-micro text-amber-200">
                                starting
                              </span>
                            )}
                            {onlyInThisTable.has(id) && (
                              <span className="rounded bg-sky-500/20 px-1 text-micro text-sky-200">
                                only here
                              </span>
                            )}
                            {likelyCraftOnly && (
                            ...
                            {isCraftOut && !likelyCraftOnly && (
                              <span className="rounded bg-white/10 px-1 text-micro text-white/45">
                                craft output
                              </span>
                            )}
                            {scarcity < 1 && (
                          ...
                          <div className="text-micro text-white/35">{id}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-2 py-2">
                    ...
                    <td className="px-2 py-2 text-body text-white/50">
                      {chance.toFixed(1)}%
                    </td>
                    <td className="px-2 py-2 text-body text-white/50">
                      {effective.toFixed(1)}%
                      ...
                      <div className="flex flex-wrap gap-2">
                        {onOpenItem && (
                          <button
                            ...
                            className="text-body text-signal/80 hover:text-signal"
                          >
                            Open
                          </button>
                        )}
                        <button
                          ...
                          className="text-body text-red-300/80 hover:text-red-300"
                        >
                          Remove
                        </button>
                      </div>
          ...
          {displayRows.length === 0 && (
            <p className="p-4 text-read text-white/35">No rows match this filter.</p>
          )}
        </div>

        <div className="border-t border-white/10 px-4 py-3">
          <div className="mb-2 flex flex-wrap items-end gap-2">
            <h5 className="mr-auto text-label uppercase text-white/30">
              Roll simulator
            </h5>
            <label className="flex flex-col gap-0.5 text-body">
              ...
              <input
                ...
                className="w-28 rounded border border-white/10 bg-black/40 px-2 py-1 text-body outline-none"
              />
            </label>
            <label className="flex flex-col gap-0.5 text-body">
              ...
              <input
                ...
                className="w-20 rounded border border-white/10 bg-black/40 px-2 py-1 text-body outline-none"
              />
            </label>
            <label className="flex flex-col gap-0.5 text-body">
              ...
              <div className="grid max-h-40 gap-1 overflow-y-auto sm:grid-cols-2 lg:grid-cols-3">
                {simHistogram.slice(0, 24).map((row) => (
                  <div key={row.id} className="flex items-center gap-2">
                    ...
                    <span className="w-28 truncate text-micro text-white/45">
                      {row.id}
                    </span>
                    <span className="w-14 text-right text-micro">
      ...
      {diffOpen && pendingDiff && (
        <div className="absolute inset-0 z-[2100] flex items-center justify-center bg-black/70 p-4">
          <div className="w-full max-w-lg rounded-xl border border-white/10 bg-concrete-900 p-5">
            <h4 className="mb-2 text-title text-signal">Review table changes</h4>
        ...
        <div className="absolute inset-0 z-[2100] flex items-center justify-center bg-black/70 p-4">
          <div className="w-full max-w-sm rounded-xl border border-white/10 bg-concrete-900 p-5">
            ...
            <p className="mb-4 text-read text-white/60">
              Wrote <span className="text-white/80">src/game/data/lootTables.json</span>.
